...
namespace Webb.Reports.Browser
{
	...
	public class CommandManager
	{
		...
		public string GetAttachedFilter(string strTemplateName,char cSpliter)
		{
			...
			int index = strTemplateName.IndexOf(cSpliter);

			if(index > 0)
			{
				strFilter = strTemplateName.Substring(index + 1,strTemplateName.Length - index - 1);
			}
		...
		public string GetTemplateName(string strTemplateName,char cSpliter)
		{
			...
			{
				strTemplate = strTemplateName.Substring(0, index);
			}
